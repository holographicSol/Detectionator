...
class ClassificationModelTrainer:
    ...
    def trainModel(self, num_objects, num_experiments=200, enhance_data=False, batch_size = 32, initial_learning_rate=1e-3, show_network_summary=False, training_image_size = 224, continue_from_model=None, transfer_from_model=None, transfer_with_full_training=True, initial_num_objects=None, save_full_model=False):
        ...
        if self.__modelType is "mobilenetv2":
            if continue_from_model is not None:
                model = tf.keras.applications.MobileNetV2(input_shape=(training_image_size, training_image_size, 3),
                                                          weights=continue_from_model, classes=num_objects,
                                                          include_top=True)
                if show_network_summary is True:
                    print("Training using weights from a previouly model")
            elif transfer_from_model is not None:
                base_model = tf.keras.applications.MobileNetV2(input_shape=(training_image_size, training_image_size, 3),
                                                               weights=transfer_from_model,
                                                               include_top=False, pooling="avg")

                network = base_model.output
                network = tf.keras.layers.Dense(num_objects, activation='softmax', use_bias=True)(network)
                ...
                base_model = tf.keras.applications.MobileNetV2(input_shape=(training_image_size, training_image_size, 3),
                                                               weights=None, classes=num_objects,
                                                               include_top=False, pooling="avg")
                network = base_model.output
                network = tf.keras.layers.Dense(num_objects, activation='softmax', use_bias=True)(network)
                model = tf.keras.models.Model(inputs=base_model.input, outputs=network)

        elif self.__modelType is "resnet50":
            if continue_from_model is not None:
                model = tf.keras.applications.ResNet50(input_shape=(training_image_size, training_image_size, 3),
                                                       weights=continue_from_model, classes=num_objects,
                                                       include_top=True)
                if show_network_summary is True:
                    print("Training using weights from a previouly model")
            elif transfer_from_model is not None:
                base_model = tf.keras.applications.ResNet50(input_shape=(training_image_size, training_image_size, 3),
                                                            weights=transfer_from_model,
                                                            include_top=False, pooling="avg")
                network = base_model.output
                network = tf.keras.layers.Dense(num_objects, activation='softmax', use_bias=True)(network)
        ...
        elif self.__modelType is "inceptionv3":

            if continue_from_model is not None:
                model = tf.keras.applications.InceptionV3(input_shape=(training_image_size, training_image_size, 3),
                                                          weights=continue_from_model, classes=num_objects,
                                                          include_top=True)
                if show_network_summary in True:
                    print("Training using weights from a previouly model")
            elif transfer_from_model is not None:
                base_model = tf.keras.applications.InceptionV3(input_shape=(training_image_size, training_image_size, 3),
                                                               weights=transfer_from_model,
                                                               include_top=False, pooling="avg")
                network = base_model.output
                network = tf.keras.layers.Dense(num_objects, activation='softmax', use_bias=True)(network)
            ...
            base_model = tf.keras.applications.InceptionV3(input_shape=(training_image_size, training_image_size, 3),
                                                           weights=None, classes=num_objects, include_top=False,
                                                           pooling="avg")

        elif self.__modelType is "densenet121":
            if continue_from_model is not None:
                model = tf.keras.applications.DenseNet121(input_shape=(training_image_size, training_image_size, 3),
                                                          weights=continue_from_model, classes=num_objects,
                                                          include_top=True)
                if show_network_summary is True:
                    print("Training using weights from a previouly model")
            elif transfer_from_model is not None:
                base_model = tf.keras.applications.DenseNet121(input_shape=(training_image_size, training_image_size, 3),
                                                               weights=transfer_from_model, include_top=False,
                                                               pooling="avg")
                network = base_model.output
                network = tf.keras.layers.Dense(num_objects, activation='softmax', use_bias=True)(network)
                model = tf.keras.model.Models(inputs=base_model.input, outputs=network)
        ...
        optimizer = tf.keras.optimizers.Adam(learning_rate=self.__initial_learning_rate)
        # optimizer = tf.keras.optimizers.legacy.Adam(lr=self.__initial_learning_rate)
        ...
        train_generator = train_datagen.flow_from_directory(self.__train_dir,
                                                            target_size=(training_image_size, training_image_size),
                                                            batch_size=batch_size,
                                                            class_mode="categorical")
        test_generator = test_datagen.flow_from_directory(self.__test_dir,
                                                          target_size=(training_image_size, training_image_size),
                                                          batch_size=batch_size,
                                                          class_mode="categorical")
    ...
    * loadModel() [This must be called once only before making a classification]
    ...
    def loadModel(self, classification_speed="normal", num_objects=10):
        ...
        * classification_speed (optional); Acceptable values are "normal", "fast", "faster" and "fastest"
        ...
        if classification_speed is "normal":
            self.__input_image_size = 224
        elif classification_speed is "fast":
            self.__input_image_size = 160
        elif classification_speed is "faster":
            self.__input_image_size = 120
        elif classification_speed is "fastest":
        ...
        if self.__modelLoaded is False:
            ...
            if self.__modelType is "":
            ...
            elif self.__modelType is "mobilenetv2":
                model = tf.keras.applications.MobileNetV2(input_shape=(self.__input_image_size, self.__input_image_size, 3),
                                                          weights=self.modelPath, classes=num_objects)
                ...
                try:
                    model = tf.keras.applications.ResNet50(input_shape=(self.__input_image_size, self.__input_image_size, 3),
                                                           weights=None, classes=num_objects)
                ...
                try:
                    model = tf.keras.applications.DenseNet121(input_shape=(self.__input_image_size, self.__input_image_size, 3),
                                                              weights=self.modelPath, classes=num_objects)
                    self.__model_collection.append(model)
                    self.__modelLoaded = True
                except:
                ...
                try:
                    model = tf.keras.applications.InceptionV3(input_shape=(self.__input_image_size, self.__input_image_size, 3),
                                                              weights=self.modelPath, classes=num_objects)
                    self.__model_collection.append(model)
                    self.__modelLoaded = True
                except:
    ...
    def loadFullModel(self, classification_speed="normal", num_objects=10):
        ...
        in the setModelPath() function. As opposed to the 'loadModel()' function, you don't need to specify the model
        type.
        This means you can load any Keras model trained with or without ImageAI and perform image prediction.
    ...
    def classifyImage(self, image_input, result_count=5, input_type="file"):
        ...
        if self.__modelLoaded is False:
            ...
            if input_type is "file":
                try:
                    image_to_predict = tf.keras.preprocessing.image.load_img(image_input,
                                                                             target_size=(self.__input_image_size, self.__input_image_size))
