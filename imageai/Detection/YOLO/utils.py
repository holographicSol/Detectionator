...
def correct_yolo_boxes(boxes, image_h, image_w, net_h, net_w):
    ...
    for i in range(len(boxes)):
        ...
        boxes[i].ymin = int((boxes[i].ymin - y_offset) / y_scale * image_h)-13  # todo: replace hardcoded -13
        boxes[i].ymax = int((boxes[i].ymax - y_offset) / y_scale * image_h)-13  # todo: replace hardcoded -13
...
def retrieve_yolo_detections(yolo_result, anchors, min_probability, nms_thresh, image_input_size, image_size,
                             labels_dict):
    boxes = []
...
def draw_boxes(image, box_points, draw_box, label, percentage_probability):
    xmin, ymin, xmax, ymax = box_points
    # print(f"USING draw_boxes YOLO: {label}")
    if draw_box is True:
        cv2.rectangle(image, (xmin, ymin), (xmax, ymax), color.label_color(_name=label), color.thickness())
    if label is not None:
        obj = f"{label.title()}"
        if percentage_probability is not None:
            prc = f"{percentage_probability} %"
        else:
            prc = f"Unknown %"
        cv2.putText(image, obj,
                    (xmin + color.anno_space_w(), ymin + color.anno_space_h()),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1e-3 * image.shape[0],
                    color.label_color(_name=label),
                    color.thickness())
        cv2.putText(image, prc,
                    (xmin + color.anno_space_w(), ymin + color.anno_space_h() + 28),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1e-3 * image.shape[0],
                    color.label_color(_name=label),
                    color.thickness())
