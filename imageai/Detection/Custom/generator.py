    ...
    def __getitem__(self, idx):
        ...
        for train_instance in self.instances[l_bound:r_bound]:
            ...
            if self.norm is not None:
                ...
                for obj in all_objs:
                    # print(f"USING BatchGenerator: {obj['name']}")
                    cv2.rectangle(img, (obj['xmin'], obj['ymin']), (obj['xmax'], obj['ymax']), color.label_color(_name=obj['name']), color.thickness())
                    cv2.putText(img, obj['name'], 
                                (obj['xmin'] + color.anno_space_w(), obj['ymin'] + color.anno_space_h()),
                                0, 1.2e-3 * img.shape[0], 
                                color.label_color(_name=obj['name']), color.thickness())
