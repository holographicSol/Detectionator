...
def run(generator, args, anchor_params, pyramid_levels):
    ...
    while True:
        ...
        if len(annotations['labels']) > 0:
            ...
            if args.anchors:
                draw_boxes(image, anchors[positive_indices], color.label_color(), thickness=1)
            # draw annotations on the image
            if args.show_annotations:
                # draw annotations in red
                draw_annotations(image, annotations, color=color.label_color(), label_to_name=generator.label_to_name)
                # draw regressed anchors in green to override most red annotations
                # result is that annotations without anchors are red, with anchors are green
                draw_boxes(image, annotations['bboxes'][max_indices[positive_indices], :], color.label_color())
